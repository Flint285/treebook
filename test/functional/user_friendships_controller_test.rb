...
class UserFriendshipsControllerTest < ActionController::TestCase
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @pending_friendship   = create(:pending_user_friendship, user: users(:tate), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @accepted_friendship  = create(:accepted_user_friendship, user: users(:tate), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
        @requested_friendship = create(:requested_user_friendship, user: users(:tate), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
        @blocked_friendship   = user_friendships(:blocked_by_tate)
      ...
      context "requested friendship" do
        setup do
  ...
  context "#new" do
  	...
  	context "when logged in" do
  	  ...
  	  should "assign a new user friendship to the currently logged in user" do
  	  	get :new, friend_id: users(:bill)
  	  	assert_equal users(:tate), assigns(:user_friendship).user
  	  end
      ...
      should "ask if you really want to friend the user" do
        get :new, friend_id: users(:bill)
        assert_match /Do you really want to frined #{users(:bill).full_name}?/, response.body
      end  	   	  	
  	end	
  end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @frined = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:tate), friend: @friend)
        create(:pending_user_friendship, friend: users(:tate), user: @friend)
        sign_in users(:tate)
      end
  ...
  context "#edit" do
    ...
    context "when logged in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:tate))
        sign_in users(:tate)
   ...
   context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:tate))
        create(:accepted_user_friendship, friend: users(:tate), user: @friend)
